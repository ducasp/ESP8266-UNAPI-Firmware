...
int CertStore::initCertStore(FS &fs, const char *indexFileName, const char *dataFileName, bool bForceNewIndex) {
  int count = 0;
  uint32_t offset = 0;
  File index;
  ...
  //If index file already exists, why do it again? Only do it again if forced
  if (!bForceNewIndex)
  {
	  index = _fs->open(_indexName, "r");
	  if (index)
	  {
		  //File already exists, just check it size, divide it by structure size, and you have the number of certificates
		  count = (index.size() / sizeof(CertStore::CertInfo));
		  index.close();
		  return count;
