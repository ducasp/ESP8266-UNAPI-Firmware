...
class WiFiClientSecure : public WiFiClient {
  public:
	...
	int connect(IPAddress ip, uint16_t port, uint16_t localPort) override;
    int connect(const String& host, uint16_t port) override;
    int connect(const char* name, uint16_t port) override;
	int connect(IPAddress ip, uint16_t port, const char* name);
	int connect(IPAddress ip, uint16_t port, uint16_t localPort, const char* name);
